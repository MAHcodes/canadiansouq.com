...
const Product = ({ product }: Props) => {
  ...
  return (
    <div className="container flex flex-col items-stretch gap-6 overflow-x-hidden">
      <ImagesSlider
      ...
      <div className="flex items-stretch gap-2">
        <a
          href={`https://wa.me/+96181921320/?text=${process.env.HOST || ""}${router.asPath}`}
          className="flex-1"
        >
      ...
      <div className="grid grid-cols-productInfo gap-x-6 gap-y-3">
        ...
        <h2 className={`${product.attributes.availability! > 0 ? "text-success" : "text-danger"} font-bold`}>{product.attributes.availability! > 0 ? "In Stock" : "Out of Stock"}</h2>
...
const Title = ({text, className}: {text: string, className?: string}) => (
  <h4 className={`text-gray font-bold ${className}`}>{text}:</h4>
)
